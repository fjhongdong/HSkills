...
| 具体服饰 | 必需 | 上装、下装、配饰的具体款式、颜色、材质（见下方详细说明） |
| 穿搭风格 | 推荐 | 街头潮流/日系清新/韩系精致/运动休闲（作为整体风格标签） |
| 性格特征 | 推荐 | 内向安静/外向开朗/酷酷的/元气满满 |
| 外貌特征 | 可选 | 长发/短发/戴眼镜/高挑/娇小 |

**具体服饰说明（必需）**：

用户必须提供角色的具体穿着单品，而非仅给出风格标签。需要包含：

| 服饰部位 | 要求 | 示例 |
|----------|------|------|
| 上装 | 款式+颜色+材质（可选设计细节） | 黑色oversized卫衣、米色棉质慵懒风落肩卫衣 |
| 下装 | 款式+颜色 | 卡其色工装裤、灰色针织休闲裤 |
| 配饰 | 有则提供，无则注明"无" | 银色项链、无 |
| 鞋履 | 有则提供，无则可省略 | 白色帆布鞋 |

> 如果用户仅提供了风格标签（如"街头潮流"）而未给出具体单品，应提示用户补充："请提供角色的具体穿着单品（如上装款式和颜色、下装款式和颜色），而非仅给出风格名称。具体服饰信息对改写准确性至关重要。"

**验证规则：**
- 角色描述少于 10 个字 → 提示用户："角色描述过于简短，请补充性别、年龄段、气质风格等信息，以便准确改写。"
- 缺少性别/年龄段/气质 → 提示用户补充，或根据已有信息合理推断（需明确告知用户是推断）
- 缺少具体服饰 → 提示用户："请提供角色的具体穿着单品（如上装款式和颜色、下装款式和颜色），而非仅给出风格名称。具体服饰信息对改写准确性至关重要。"
- 仅有风格标签但无具体单品 → 根据风格标签合理推断具体服饰，但需明确告知用户是推断，并建议用户确认或调整
...
| `subjects[].clothing` | 全部替换为用户提供的具体服饰（top/bottom/accessories/overall_style），直接使用用户指定的款式、颜色、材质，不自行编造 | 米色慵懒风卫衣 → 黑色oversized卫衣 + 卡其色工装裤 |
...
| 缺少具体服饰 | "请提供角色的具体穿着单品（如上装款式和颜色、下装款式和颜色），而非仅给出风格名称。具体服饰信息对改写准确性至关重要。" |
...
- `clothing` 替换时使用用户提供的具体服饰（上装/下装/配饰的款式、颜色、材质），填入对应的 `top`、`bottom`、`accessories` 字段，`overall_style` 使用用户提供的穿搭风格标签
...
- 具体服饰（必需）：
  - 上装：[款式+颜色+材质，如 黑色oversized卫衣]
  - 下装：[款式+颜色，如 卡其色工装裤]
  - 配饰：[如 银色项链，无则填"无"]
  - 鞋履：[如 白色帆布鞋，可选]
- 穿搭风格：[街头潮流/日系清新/韩系精致/运动休闲/...]（推荐）
