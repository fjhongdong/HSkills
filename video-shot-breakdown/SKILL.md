...
| 15-30秒 | 每1-2秒1帧 | 20帧 | 常见短视频时长 |
| 30-60秒 | 每2-3秒1帧 | 25帧 | 中等长度视频 |
| 60-120秒 | 每3-5秒1帧 | 30帧 | 较长视频 |
| > 120秒 | 每5秒1帧 | 40帧 | 长视频，需控制帧数 |

**ffmpeg提取命令**
```bash
# 根据视频时长自动计算间隔
ffmpeg -i video.mp4 -vf "fps=1/2,scale=1280:-1" -q:v 2 frames/frame_%03d.jpg
```

**场景变化检测**（捕捉转场点）
```bash
# 检测场景变化并提取关键帧
ffmpeg -i video.mp4 -vf "select='gt(scene,0.3)',scale=1280:-1" -vsync vfr -q:v 2 frames/frame_%03d.jpg
```

**执行方式：**
1. 提取关键帧图片序列
2. 将关键帧按顺序传给多模态模型
3. 模型基于关键帧推断镜头信息和画面内容
4. 注意：运镜和转场需要根据帧间差异推断，可能不够准确
...
**使用方法：**
```bash
python scripts/extract_frames.py <视频路径> [输出目录] [--mode <basic|scene>]
```
...
2. **分析精度差异**：
   ...
   - 关键帧分析：运镜和转场依赖帧间推断，可能存在误差
3. **视频质量**：视频清晰度越高，分析结果越准确
4. **时长限制**：建议分析2分钟以内的短视频，直接视频分析效果最佳
5. **隐私保护**：用户上传的视频仅用于分析，不会被存储或用于其他用途
6. **分析准确性**：AI分析可能存在误差，关键信息建议人工复核
