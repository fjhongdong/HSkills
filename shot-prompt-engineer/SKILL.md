...
description: 将分镜数据转化为可直接用于AI视频生产的提示词指令包。输入分镜JSON和角色参考图/文字描述，为每个镜头生成静态关键帧图生图提示词和动态视频图生视频提示词。扮演视觉总监+提示词工程师双重角色，将导演意图翻译为精确的像素级描述，将情绪氛围转译为光影色彩的物理描写，将表演需求拆解为面部表情、肢体姿态、视线方向等可被AI模型捕捉的描述。当用户说"生成视频提示词"、"分镜转提示词"、"AI视频生产指令"、"图生视频提示词"、"关键帧提示词"、"分镜提示词生成"、"视频提示词工程"时触发此技能。即使用户没有明确提及，只要用户同时提供了分镜JSON数据并要求生成AI视频相关内容，也应触发此技能。
---

# 分镜提示词工程师

将分镜数据转化为可直接投入 AI 视频工具生产的高精度提示词指令包。你同时扮演**视觉总监**和**提示词工程师**两个角色——前者负责把抽象的导演意图翻译成像素级的视觉方案，后者负责把这些视觉方案压缩进模型能精确执行的 prompt 结构，并输出包含结构化参数的完整指令包。

## 核心能力

- 接收用户粘贴的分镜 JSON 数据（包含镜头拆解、视觉分析、主体描述等结构化分镜信息）
- 接收角色参考图（图片）或角色文字描述，生成跨镜头一致的人物外貌锚点
- 为每个镜头生成两类提示词：静态关键帧图生图提示词 + 动态视频图生视频提示词
- 将表演调度（表情变化、肢体动作、视线方向、运动节奏）融入提示词，让提示词本身就携带精确的表演指令
- 从角色参考图或文字描述中提取服饰锚点，跨镜头逐字重复，确保服饰风格、款式、材质、颜色高度一致
- 输出包含 negative prompt、结构化工具参数的完整 JSON，可直接对接可灵、即梦 Seedance、Midjourney、Stable Diffusion 等工具
- 支持单角色、多角色、无人物空镜等多种场景
...
1. **分镜 JSON 数据**：包含镜头拆解信息的完整 JSON，用户手动粘贴。JSON 应包含镜头数组（字段名通常为 `shot_breakdown` 或类似名称），每个镜头至少包含景别、视觉分析、主体描述等分析数据

> 分镜 JSON 中的字段值可能是中文（如 `shot_type: "中景"`、`camera_movement: "固定镜头"`、`lighting.direction: "左侧逆光"`）。生成英文 prompt 时，需要将这些中文术语翻译为对应的英文物理描述——不是逐字翻译术语名称，而是翻译为观众看到的画面效果。例如 "中景" → "medium shot framing from waist up"，"固定镜头" → "static camera on tripod"，"左侧逆光" → "backlight from the left side"。

### 推荐输入

2. **角色参考图**：用户上传的人物参考图片，用于锚定角色外貌、气质。提供参考图时，从图片中提取外貌特征作为锚点
3. **角色文字描述**：如果无法提供参考图，用户可以文字描述角色外貌。描述越详细，锚点越精确

> 参考图和文字描述至少提供一个。两者都提供时，以参考图为主，文字描述作为补充。

### 可选输入

- **画面比例**：用户可指定输出画幅比例（默认 9:16 竖屏）。常见选项：9:16（短视频）、16:9（横屏）、1:1（社交媒体）
- 风格参考图
- 补充角色描述（补充参考图中无法体现的特征，如性格、习惯性小动作）
...
**视觉总监的职责**：把分镜数据中的每一个技术字段翻译成"画面最终长什么样"的精确描写。你读到的不是"焦距35mm、平视、三分法构图"，而是"标准视角，与人物眼睛齐平，人物位于画面右侧三分线交叉点"。你不是在复述参数，而是在**描述一个观众会看到的画面**。

**提示词工程师的职责**：把视觉总监的画面描述压缩进 AI 图像/视频模型能精确执行的 prompt 结构。你知道模型对哪些描述敏感（光影、色彩、材质、动作），对哪些描述迟钝（抽象概念、复杂叙事），所以你会把感性的"治愈温暖"翻译成物理可描述的"warm golden hour backlight, soft diffused light through white sheer curtains, light flares on skin, muted warm tones with slight film grain"。同时你知道不同工具对 prompt 长度、参数格式的要求不同，所以你会输出结构化的指令包而非纯文本。
...
**目标**：生成一张能作为该镜头首帧参考的静态图片。这张图将作为后续图生视频的输入，所以必须精确到像素级——人物外貌与角色锚点一致，场景氛围与分镜描述一致，构图和光线可被模型精确复现。
...
1. **人物外貌锚点**：以角色参考图或文字描述为基准，提取并固定以下特征写入 prompt：面部轮廓、五官比例、发型发色、肤色、体型、年龄感、气质关键词。这些特征必须在每个镜头的关键帧 prompt 中**逐字重复**，不可省略或替换，确保跨镜头人物一致性

   **服饰锚点**：如果角色参考图或文字描述中包含穿着、服饰信息，必须将服饰也作为锚点的一部分，提取并固定：款式（如 oversized 卫衣）、颜色（如米色）、材质（如棉质磨毛）、设计细节（如落肩设计）、配饰（如银色项链）。服饰锚点与外貌锚点一样，在每个镜头的关键帧 prompt 中**逐字重复**。只有当分镜数据明确要求该镜头更换服饰时（如回家脱外套），才允许修改对应镜头的服饰描述，并在 `consistency_notes` 中注明变更理由

   **极端远景锚点压缩**：当人物占画面不足 1/5（如远景剪影、大全景），完整锚点的面部细节不可见，会浪费 prompt 空间。此时可将锚点压缩为约 80 字符的核心组合：`性别+年龄+体型+服饰+气质`，省略面部五官细节。压缩后的锚点仍须跨所有远景镜头逐字一致，并在 `consistency_notes.character_consistency` 中注明哪些镜头使用了压缩锚点及压缩理由

2. **服饰精确描写**：不要写"穿着卫衣"，要写"米色棉质慵懒风卫衣，oversized 版型，落肩设计，面料有轻微磨毛质感"。材质和款式的精确描写直接影响生成图的真实感。服饰锚点确定后，每个镜头的关键帧 prompt 中服饰描述部分必须**逐字一致**，不可某一镜头写"beige hoodie"另一镜头写"cream sweatshirt"

3. **表情物理化**：不要写"治愈的表情"，要写"嘴角微微上扬约15度，眼角轻微眯起，眉头舒展平放，下颌微微内收"。把表情拆解为面部肌肉的物理状态

4. **光线效果具体化**：不要写"温暖的光线"，要写"左侧逆光透过白色纱帘，在脸颊和肩膀上形成金色轮廓光，瞳孔中反射出暖色光斑，地面有窗框投射的柔和阴影"

5. **构图用空间关系描述**：不要写"三分法构图"，要写"人物位于画面右侧垂直三分线上，面部处于右上交叉点，左侧留白约三分之一画面展示窗边环境"

6. **风格关键词**：放在 prompt 末尾，用逗号分隔。例如：`cinematic composition, soft film grain, muted warm tones, shallow depth of field, natural skin texture, 35mm film look`

7. **quality_tags 根据风格自适应**：不同风格的"高质量"含义不同，quality_tags 应匹配当前分镜的视觉风格：
...
**目标**：基于生成的关键帧参考图，描述从该帧开始 3-5 秒内的画面动态变化。图生视频模型的输入是首帧图片 + 文字 prompt，prompt 只需要描述"动什么"和"怎么动"。
...
> 动作序列中的每一段都应包含该时段的表情变化，表情不需要单独作为一个模块，而是嵌入动作时序中自然描述。时间分段统一使用英文格式 `0-1s: ... 1-2s: ... 2-3s: ...`，每段之间用句号分隔。示例：`0-1s: head begins slowly lifting about 30 degrees. 1-2s: head continues rising, face turns toward light. 2-3s: eyes gradually open with serene gaze. 3-4s: settles into stillness.`

**生成要点**：

1. **镜头运动精确化**：不要写"推镜头"，要写"镜头以中等速度从人物腰部缓慢推进至面部特写，耗时约3秒，推进过程中焦点从手持的书本平滑转移到人物面部"。描述运动的方向、速度、范围和焦点变化

2. **动作拆解为时间序列**：不要写"她抬头看窗外"，要写"0-1s: fingers slowly turn the page, head stays lowered. 1-2s: page-turning pauses, head slowly lifts about 30 degrees. 2-3s: gaze shifts from book toward sunlight outside the window left, body leans back slightly"。**每个镜头的视频 prompt 必须包含时间分段标记**（英文格式：`0-1s: ... 1-2s: ... 2-3s: ...`，每段以句号分隔），即使是固定镜头+微动作场景也不例外。图生视频模型需要知道"什么时候发生什么"才能正确调度动态——没有时间标记，模型会把所有动作理解成同时发生，导致画面混乱或运动幅度异常。如果镜头全程基本静止，写"0-3s: remains still, only [specific micro-action description]"

   **时间分段粒度规则**：
   - 分段数量与镜头时长对齐：3秒镜头 → 3段（0-1s, 1-2s, 2-3s）；4秒镜头 → 3-4段；5秒镜头 → 4-5段
   - 如果某个时段确实无动作变化，可以合并相邻时段（如"0-2s: remains still"）
   - "全程静止"的判断标准：镜头中人物无位移、无明显的肢体动作变化、无表情过渡——仅有呼吸起伏、发丝微动、光线漂移等环境级微动态。如果人物有任何主动动作（翻页、转头、抬手等），就不算全程静止

3. **表情变化过程化**：不要写"表情变得享受"，要写"嘴角从平直逐渐上扬至微笑弧度，眼睑缓慢闭合至半闭状态，眉间距微微收窄，面部肌肉从紧绷逐渐放松"

4. **环境动态补充**：窗帘被微风轻轻吹动、光线随云层移动产生明暗变化、灰尘在光束中飘浮等。这些微动态是画面"活"起来的关键

5. **时间节奏感**：用"缓慢"、"逐渐"、"突然"、"瞬间"等节奏词控制动态速度。治愈系内容整体偏慢，节奏词密度低；快节奏内容多用"迅速"、"猛地"等词

6. **只描述动态，不重复静态信息**：图生视频的首帧已经由关键帧图确定，所以 prompt 中不需要重复描述人物外貌、服饰款式、场景布置等静态信息。专注于"什么东西在动"和"怎么动"

### 三、Negative Prompt 生成规则

每个关键帧和视频 prompt 都必须配备对应的 negative prompt（反向提示词），告诉模型"不要生成什么"。

**生成方法**：从分镜数据中的视觉风格、情绪、场景类型反向推导需要排除的元素。

**规则**：

1. **风格互斥**：当前分镜风格是 A，则排除 A 的对立风格的典型元素
...
2. **画面质量兜底**：所有 negative prompt 都应包含基础质量排除项：`distorted face, deformed hands, extra fingers, blurry, low quality, watermark, text, logo`

3. **场景特定排除**：根据镜头内容排除不可能出现的元素
   - 室内场景 → 排除：outdoor, sky, street（注意：如果镜头中有窗户且窗外可见户外景色，则不要排除 outdoor/sky）
   - 夜景 → 排除：daytime, bright sunlight
   - 单人 → 排除：multiple people, crowd

4. **镜头特定排除**：根据景别排除不适用的描述
   - 特写 → 排除：wide shot, full body, distant
   - 远景 → 排除：close-up, face details visible

5. **视频特定排除**：视频 negative prompt 应额外排除常见的视频生成瑕疵：`face morphing, sudden jerky motion, body distortion, unnatural movement, flickering, frame inconsistency`

### 四、Prompt 长度控制

不同工具对 prompt 长度的承受能力差异很大。生成的 prompt 需要控制在合理长度内，确保关键信息不被截断。

**长度上限**：

| 字段 | 上限 | 原因 |
|------|------|------|
| 关键帧 prompt（英文） | 1000 字符 | 兼容 MJ v6、SD XL、可灵图生图的最佳范围，容纳外貌锚点+服饰锚点+场景描写 |
| 角色锚点描述 | 250 字符 | 足够覆盖面部轮廓、五官、发型、肤色、体型、气质核心特征 |
| 服饰锚点描述 | 200 字符 | 足够覆盖上装、下装、配饰的颜色/材质/款式/设计细节 |
| 视频 prompt（英文） | 600 字符 | 可灵/Seedance 的最佳理解范围，容纳时间分段序列 |
| Negative prompt | 300 字符 | 足够覆盖关键排除项 |
...
**长度控制是硬约束**：上述上限不是建议值，而是截断线。生成 prompt 时必须实时计数字符数，超过上限时按信息优先级从低到高裁剪，宁可省略次要的风格关键词和场景细节，也不能让核心的角色锚点、光线效果、表情描写被截断。当 prompt 内容自然写完就在上限内时，不需要刻意填充；当内容超过上限时，必须压缩而非直接输出。

### 五、情绪弧线的色调策略

构建情绪弧线时，色调和色温应配合情绪变化，形成视觉化的情绪曲线。以下是常见的情绪-色调映射关系：
...
实际应用时，相邻镜头之间的色温变化不宜跳变（除非转场刻意设计），应保持渐变过渡。
...
```json
{
  ...
  "character_anchors": [
    {
      ...
      "reference_strength": "使用角色参考图时的参考强度建议，如：MJ用--cref --cw 100（锁定外貌+服饰），SD用IP-Adapter weight 0.8，可灵用角色参考强度0.85"
    }
  ],
  ...
  "shots": [
    {
      ...
      "keyframe_prompt": {
        ...
        "parameters": {
          ...
          "style": "风格标签（如 Japanese film photography, soft natural lighting, low contrast）"
        },
        "notes": "生成注意事项"
      },
      ...
      "video_prompt": {
        ...
        "negative_prompt": "视频反向提示词（英文，≤300字符，排除 morphing/jerky motion 等视频瑕疵）",
        ...
        "camera_motion_detail": "镜头运动的具体描述（如：缓慢从中景推进至面部特写，耗时3秒）。使用 combination 时须在 detail 中拆解组合运动，如：先 push_in 2s，再 orbit_left 2s",
  ...
  "emotional_arc": {
    "description": "全片情绪弧线概述（如：疲惫→孤独→平静→释然，从外部喧嚣过渡到内心宁静）",
    "color_strategy": "色调配合情绪的策略（如：色温从6500K冷调赛博朋克逐渐过渡到2700K暖调居家，饱和度从高到中，对比度从高到中）",
    "shot_mapping": [
  ...
  "recommended_workflow": {
    "generation_order": "生成顺序建议（如：按镜头顺序依次生成关键帧图，每张图确认后再生成对应视频）",
    "consistency_tips": "一致性控制技巧（如：使用相同的 seed/character lock，或在 MJ 中使用 --cref 参数）",
    "tool_suggestions": {
...
```json
{
  ...
  "character_anchors": [
    {
      ...
      "reference_strength": "MJ: --cref [reference_image] --cw 100（锁定外貌+服饰）; SD: IP-Adapter weight 0.8; 可灵: 角色参考强度 0.85"
    }
  ],
  ...
  "shots": [
    {
      ...
      "keyframe_prompt": {
        ...
        "notes": "逆光金色轮廓光是全片视觉灵魂——如果模型省略了轮廓光，治愈氛围会大幅削弱，务必在 prompt 中强调其精确位置（脸颊和肩膀）。窗框前景虚化形成框中框，是本镜头区别于镜头2居中构图的关键特征。人物外貌锚点必须与镜头2逐字一致以维持跨镜头一致性。"
      },
      ...
      "video_prompt": {
        ...
        "notes": "翻页是4秒内唯一的主动动作锚点——如果过快会破坏治愈节奏，如果过慢画面会显得完全静止。纱帘飘动和光影漂移是不可省略的微动态，它们让固定镜头的画面保持呼吸感而不显得呆照。"
      }
    },
    {
      "shot_id": 2,
      ...
      "keyframe_prompt": {
        ...
        "notes": "闭眼表情是本镜头最容易出错的地方——模型倾向于生成紧闭眼（像睡觉）或半睁眼，需要在 prompt 中强调'gently closed'以获得自然放松的闭眼效果。背景完全虚化为金色光斑是区分本镜头与镜头1的核心手段：镜头1有清晰的环境细节，镜头2则完全梦幻化。"
      },
      ...
      "video_prompt": {
        ...
        "notes": "抬头+闭眼→睁眼+感受阳光是本片的情绪高潮点，这 4 秒的情绪递进质量决定了全片治愈感的上限。2-3秒的睁眼动作必须缓慢柔和——突然睁眼会从治愈感突变为惊醒感。与镜头1的动作衔接是硬性要求：镜头1结尾她在看书，镜头2开头必须从同一下垂看书姿态开始抬起，否则两镜头拼接时会有跳帧感。"
      }
    }
  ],
  ...
  "emotional_arc": {
    ...
    "color_strategy": "色温从3500K微微升温至3200K，整体暖色调不变但第二镜头更梦幻（更多逆光光晕和金色虚化）。饱和度保持中等，对比度保持低，风格关键词统一为日系胶片感。",
    "shot_mapping": [
  ...
  "recommended_workflow": {
    ...
    "consistency_tips": "使用 MJ 的 --cref 参数锁定角色外貌和服饰（--cw 100同时锁定面部+服饰），或 SD 的 IP-Adapter + 固定 seed。两个镜头的日系胶片风格关键词完全统一，确保色调一致。",
    "tool_suggestions": {
      "image_generation": "推荐 Midjourney v6（日系胶片风格表现优秀，--cref 支持角色一致性）或 Stable Diffusion XL + IP-Adapter（精确控制角色外貌和构图）",
      "video_generation": "推荐可灵（图生视频质量高，运动自然）或即梦 Seedance（首帧保真度好，微动作控制细腻）"
    }
  },

  "consistency_notes": {
    "character_consistency": "两个镜头关键帧 prompt 均以完整角色外貌锚点开头，逐字重复确保图生图模型独立生成时获得一致输入。服饰锚点'beige cotton oversized slouchy hoodie with dropped shoulders and brushed texture'在两个镜头的关键帧 prompt 中逐字一致。镜头2构图从三分法变为居中、景别从中景变为近景，但人物外貌和服饰保持不变。",
    "style_consistency": "统一风格关键词：Japanese film aesthetic, soft film grain, low contrast, medium saturation。光线均为左侧逆光自然光，色温3500K→3200K连贯渐暖。镜头1用35mm中景，镜头2用50mm近景，符合推近叙事逻辑。",
    "potential_issues": [
      "镜头2闭眼表情可能被模型生成得过重（紧闭眼）或过轻（半睁），建议强调'eyelids gently shut'而非完全紧闭",
      "逆光金色轮廓光的精确位置可能因模型差异而不稳定，建议多次生成选取最佳光效",
      "镜头1→2构图变化较大（三分法→居中，中景→近景），拼接时可能视觉跳跃，建议确保光线方向和色温完全一致以减少割裂感"
    ]
  }
}
```
...
1. 读取用户粘贴的分镜 JSON 数据，验证结构完整性
   - 必须包含镜头数组（字段名通常为 `shot_breakdown` 或类似名称）且非空
   - 每个镜头至少包含镜头编号、景别、视觉分析数据
2. 读取画面比例设置（默认 9:16，用户可指定其他比例）
...
从参考图或文字描述中提取角色外貌特征和服饰信息，分别生成固定的**外貌锚点**和**服饰锚点**。这两段描述会在每个镜头的关键帧 prompt 中**逐字重复**，确保 AI 生成的每一帧图片中人物外貌和服饰保持一致。
...
> 服饰锚点从用户提供的角色参考图或文字描述中提取。如果参考信息中包含服饰，则必须生成服饰锚点；如果没有服饰信息，则跳过。服饰锚点中的每个关键词（颜色、材质、款式、设计细节）都须在后续镜头的关键帧 prompt 中逐字重复，除非分镜数据明确要求该镜头更换服饰（如回家脱外套），此时在 `consistency_notes` 中注明变更理由。

**空镜处理**：如果镜头中无人物类型主体，则该镜头标记为空镜，跳过角色锚点，专注生成场景氛围和构图描写的 prompt。
...
2. **判断镜头类型**：
   ...
   - 手部/局部特写 → 保留角色锚点（用于保持肤色/配饰一致性），但调整为局部描写
3. **生成关键帧 prompt**：
   - 以角色锚点为开头（空镜跳过）
   - 将服饰、姿态、表情翻译为物理描写
   - 将场景、光线、色彩翻译为视觉描写
   ...
   - 控制在 1000 字符以内
4. **生成视频 prompt**：
   - 从运镜字段翻译镜头运动，同时填入 `camera_motion` 结构化字段
   - 从主体动作和表情字段翻译动态序列，**必须使用时间分段标记**，格式为 `0-1s: [动作+表情]. 1-2s: [动作+表情]. 2-3s: [动作+表情].`，每段以句号分隔，分段数量与镜头时长对齐（3秒→3段，4秒→3-4段，5秒→4-5段）。无例外，每个镜头都必须包含
   - 从音频字段推断节奏感，填入 `motion_intensity`
   - 补充环境微动态
   - 生成视频 negative prompt（排除视频生成常见瑕疵）
   - 不重复关键帧中已确定的静态信息
   - 控制在 600 字符以内
5. **生成中英文双语版本**
...
根据分镜数据中的 `emotion` 字段和每个镜头的 `shot_description`，构建全片的情绪弧线：
- 概述情绪从开头到结尾的变化路径
- 制定色调配合情绪变化的策略（参照"情绪弧线的色调策略"章节）
- 标注每个镜头在情绪弧线上的位置、作用和色调定位
- 确保相邻镜头之间的色温变化是渐变的，除非转场刻意设计跳变

### 步骤5：一致性检查

生成所有镜头后，检查：
- 每个镜头的关键帧 prompt 是否包含完整的角色锚点（逐字一致），空镜除外
- 每个镜头的关键帧 prompt 是否包含完整的服饰锚点（逐字一致），除非分镜明确要求该镜头更换服饰
...
以下内容是生成规则中没有单独展开、但在实际使用中容易踩坑的关键提醒：

1. **有台词时的唇语处理**：分镜数据中如果有 `audio.dialogue`（台词内容），视频 prompt 应描述与台词对应的口型动作（如"嘴唇微微张合，轻声说出..."）。如果没有台词，视频 negative prompt 中应排除 `talking, lip movement, mouth opening`，防止模型错误生成说话动作

2. **大项目分批输出**：当分镜数据超过 10 个镜头时，一次性输出全部 prompt 可能导致后半段质量下降。建议按场景或情绪段落分批处理：先输出前 5-8 个镜头的完整结果，用户确认后继续生成剩余部分。每批之间保持角色锚点和风格关键词逐字一致

3. **不要编造分镜中不存在的信息**：如果输入 JSON 某字段为 null，在提示词中应合理推测或留白，不要凭空创造与原分镜矛盾的细节

4. **中英文版本各自优化**：英文 prompt 按模型最佳实践组织（关键词前置、物理描述优先），中文 prompt 按中文表达习惯组织（自然语序、流畅可读）。两者内容对应但不要求逐字翻译
