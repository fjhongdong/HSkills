#!/usr/bin/env python3
"""Grade the script outputs - Iteration 6."""
...
def grade_script(script_content, eval_metadata):
    ...
    for assertion in eval_metadata["assertions"]:
        ...
        if name == "包含批量热点分析报告":
            ...
            checks = ["反转", "反差", "视觉符号", "时间跳跃", "创意转场", "情绪转化"]
            found = sum(1 for c in checks if c in script_content)
            if found >= 1:
                passed = True
                evidence = f"包含创新元素（{found}项）"
            else:
                evidence = "缺少创新元素"
            ...
            # Check for unique elements like unexpected scenes, visual symbols, emotional contrast
            unique_elements = [
                "深夜", "泡面", "外卖", "雨", "黄昏", "地铁", "加班",
                "反转", "意外", "独特", "记忆点", "符号"
            ]
            found = sum(1 for e in unique_elements if e in script_content)
            if found >= 1:
                passed = True
                evidence = f"脚本有独特记忆点"
            else:
...
def main():
    base_path = "/Users/Abner/.claude/skills/hot-script-generator/workspace/iteration-6/batch-hotspots"
