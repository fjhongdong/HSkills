# shot-script-rewriter 实现计划

> **面向 AI 代理的工作者：** 必需子技能：使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务实现此计划。步骤使用复选框（`- [ ]`）语法来跟踪进度。

**目标：** 创建 shot-script-rewriter Skill，接收 video-shot-breakdown JSON + 角色文字描述，最小化改写分镜脚本。

**架构：** 独立 Skill，仅包含 SKILL.md 文件。AI 直接执行改写逻辑，无辅助脚本。输入输出格式与 video-shot-breakdown 完全兼容。

**技术栈：** Claude Code Skill（SKILL.md 提示词工程）

**规格文档：** `docs/superpowers/specs/2026-03-29-shot-script-rewriter-design.md`
...
| 操作 | 文件 | 职责 |
|------|------|------|
| 创建 | `shot-script-rewriter/SKILL.md` | Skill 定义，包含触发条件、改写规则、执行流程、示例 |
| 修改 | `CLAUDE.md:44` | Skill 分类表新增 shot-script-rewriter |

---

### 任务 1：创建 SKILL.md — Frontmatter 与核心能力

**文件：**
- 创建：`shot-script-rewriter/SKILL.md`

- [ ] **步骤 1：创建目录和文件**

创建 `shot-script-rewriter/SKILL.md`，写入 frontmatter 和核心能力部分：
...
确认 `shot-script-rewriter/SKILL.md` 存在且 frontmatter 格式正确（`---` 包裹、name 和 description 字段完整）。

---

### 任务 2：创建 SKILL.md — 改写规则

**文件：**
- 修改：`shot-script-rewriter/SKILL.md`（追加内容）
...
确认三级规则完整，字段路径与 video-shot-breakdown 的 JSON 输出结构一致。

---

### 任务 3：创建 SKILL.md — 输入规范
...
确认输入规范完整覆盖 JSON 输入要求和角色描述格式。

---

### 任务 4：创建 SKILL.md — 执行流程
...
确认错误处理表与规格一致，注意事项覆盖了所有关键约束。

---

### 任务 6：创建 SKILL.md — 改写示例
...
确认示例中的改写字段路径与 video-shot-breakdown 的 JSON 结构一致，三级分类清晰。

---

### 任务 7：更新 CLAUDE.md 分类表
...
在 `CLAUDE.md` 的 Skill 分类表中，将 `shot-script-rewriter` 加入"短视频创作"行，排在 `video-shot-breakdown` 之后：
...
确认分类表格式对齐，shot-script-rewriter 出现在正确位置。
...
**文件：**
- 检查：`shot-script-rewriter/SKILL.md`（全文）
- 检查：`CLAUDE.md`（分类表）

- [ ] **步骤 1：全文检查 SKILL.md**

验证以下内容：
- Frontmatter 格式正确（`---` 包裹、name、description 字段完整）
- 八个章节完整：核心能力（含触发区分）、改写规则、输入规范、执行流程、错误处理、注意事项、示例
- 所有 JSON 字段路径与 video-shot-breakdown 的输出格式一致
- 示例中的 JSON 语法正确
...
```bash
mkdir -p shot-script-rewriter
git add shot-script-rewriter/SKILL.md CLAUDE.md
git commit -m "feat: 新增 shot-script-rewriter Skill — 分镜脚本角色改写器"
```
